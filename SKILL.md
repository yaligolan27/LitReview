...
- בקשות `interview` / `interview_charter` הן ראיון העומק שלפני הצינור:
  שיחה עם המשתמש דרך הממשק. הן לא דורשות חיפוש web — ענה מהר ולפי ה-system
  שבבקשה (מראיין בלבד: שאלות והעמקה, בלי לכתוב תוכן של הסקר). תור `interview`
  חייב לחזור בפורמט ה-JSON שב-system — שאלות עם 3-5 אפשרויות מוכנות ללחיצה,
  מותאמות לנושא ולתשובות הקודמות (הממשק מציג אותן ככפתורים).
