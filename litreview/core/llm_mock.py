...
_WRITER_TEMPLATE = """[CALLOUT:blue]בפשטות: הפרק מסביר מה ידוע היום בתחום, אילו גישות קיימות ומה עוד לא פתור. לא צריך רקע מוקדם כדי להבין את עיקרי הדברים.[/CALLOUT]

בפרק זה נסקרת הספרות המרכזית בתחום, תוך הבחנה בין הגישות המרכזיות שהתפתחו בשנים האחרונות. המחקר המוקדם ביסס את המסגרת המושגית של התחום והגדיר את שאלות היסוד [1]. בהמשך, עבודות מאוחרות יותר הרחיבו את היריעה ובחנו את הסוגיה ממגוון זוויות מתודולוגיות [2].
...
def complete(prompt: str, purpose: str, system: str = "") -> str:
    ...
    if purpose == "glossary":
        return json.dumps({"terms": [
            {"term": "סקירה שיטתית (Systematic Review)",
             "plain_definition": "סקירה שמאתרת ומסננת מחקרים לפי שיטה מוגדרת מראש."},
            {"term": "רמת בשלות טכנולוגית (TRL)",
             "plain_definition": "סולם 1-9 שמתאר כמה טכנולוגיה קרובה לשימוש מעשי."},
            {"term": "עיגון טענות (Grounding)",
             "plain_definition": "בדיקה שכל טענה בטקסט באמת נתמכת במקור שצוטט."},
        ]}, ensure_ascii=False)
