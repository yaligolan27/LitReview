...
[CALLOUT:blue]נקודת מפתח: הספרות מבחינה בין הרמה התיאורטית לרמה היישומית — ההבחנה הזו חוזרת לאורך כל הפרק.[/CALLOUT]
...
לסיכום ביניים, **הכיוון המסתמן בספרות** הוא שילוב בין גישות משלימות, תוך הקפדה על מתודולוגיה שקופה וניתנת לשחזור [1,4]. הפרקים הבאים מרחיבים את הדיון בהיבטים היישומיים של הנושא.
"""
...
def complete(prompt: str, purpose: str, system: str = "") -> str:
    ...
    if purpose.startswith("writer:ch"):
        return _WRITER_TEMPLATE.format(n=_chapter_number(purpose))
